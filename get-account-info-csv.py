...
# Indebted to
# http://stackoverflow.com/questions/10580220/read-text-in-a-file-to-check-environment-variables
...
for i in range(0, len(screen_names), 100):
    ...
    for user in users:
        ...
        outusers.append([screen_name, id, location, expanded_url, created_at, client, client_url])
        with open(input_file + '.tweets.csv', 'wb') as f:
            writer = unicodecsv.writer(f)
            writer.writerow(["screen_name", "id", "location", "expanded_url", "created_at", "client", "client_url"])
            writer.writerows(outusers)
            
        pass

print('Done! Info written to ' + input_file + '.tweets.csv')
